...
const renderCards = (products,page) => {
    cardList.innerHTML = "";
...
const tabs = (products) => {
    ...
    counter = Math.ceil(products.length / limit);  

    for (let i = 1; i <= counter; i++) {
        const paginationBtnHTML = `<a class="page-link btn btn-primary pagination-button" data-btn="${i}" href="#">${i}</a>`;
        paginationList.insertAdjacentHTML("beforeend", paginationBtnHTML);

        const paginationBtn = document.querySelector(`#pagination-list a[data-btn="${i}"]`);    
        paginationBtn.addEventListener("click", (event) => {
            event.preventDefault();
            currentOffset = i - 1;
            renderCards(products.slice(currentOffset * limit, (currentOffset + 1) * limit), i);
        });
    }
    ...
    prevBtn.addEventListener("click", () => {
        if (currentOffset > 0) {
            currentOffset--;
            renderCards(products.slice(currentOffset * limit, (currentOffset + 1) * limit), currentOffset + 1);
        }
    });

    nextBtn.addEventListener("click", () => {
        if (currentOffset < counter - 1) {
            currentOffset++;
            renderCards(products.slice(currentOffset * limit, (currentOffset + 1) * limit), currentOffset + 1);
        }
    });
